...
public class MainClass {
	...
	/*
	 Writer 1. 문자 기반의 출력 스트림이다. 
	 2. Writer로 끝나는 이름의 클래스는 모두 문자 기반의 출력 스트림이다. 
	 3. 문자를내 보내는 스트림이다.(파일로 문자를 보낸다. 서버로 문자를 보낸다. 등등)
	 */
	...
	public static void ex02() {
		...
		try (FileWriter fw = new FileWriter(file)) { // 출력 스트림 생성

			char[] cbuf = { '나', 'b', 'c', 'd', 'e' };
			String str = "abcde";

			fw.write(cbuf, 0, 2); // 인덱스 0부터 2글자만 보내시오 → 나b
			fw.write(str, 2, 3); // 인덱스 0부터 2글자만 보내시오 → cde
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void ex03() {
		...
		/*
			BufferedWriter('속도 향상'을 위한 빠른 기억장소를 보조하는..)
			1. 내부 버퍼를 가지고 있는 Writer이다.
			2. 속도 향상을 위해서 사용한다.
			3. 보조 스트림이므로 '메인 스트림과 함께 사용'해야 한다.
		*/
		...
		try {
			...
			try {
				// 보조 스트림을 사용한 경우에는 보조스트림만 닫으면 메인 스트림도 함께 닫힌다.
				if(bw != null) {
					bw.close();
				}
			} catch(IOException e) {
	...
	public static void ex04_1() { //ex03 try - catch - resources문으로 바꾸기
		...
		try(BufferedWriter bw = new BufferedWriter(new FileWriter(file))) { // 메인스트림+보조스트림 함께 사용
	...
	public static void ex04() { //ex03 try - catch - resources문으로 바꾸기
	...
	public static void ex05() {
		...
		try {
			...
			out.close(); // 이미 close 내부에 IOException을 처리하고 있다
