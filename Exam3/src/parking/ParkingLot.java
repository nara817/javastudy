...
/*1번째 방법*/			if(carNo.equals(car.getCarNo())) { //차량번호 일치 여부 판단 됨
				// 마지막 차량을 옮겨오기
				// 제거할 차량의 위치 : i
				// 마지막 차량의 위치 : idx-1
				cars[i] = cars[idx-1]; //cars[i] = cars[idx-1] = cars[i] = cars[--] 동일 
				cars[--idx] = null;
				return;
			}
		}
...
/*1번방법*/for (int i = 0; i < idx; i++) {
			Car car = cars[i];
			System.out.println(car);
		}
/*2번방법*/		for (int i = 0; i < cars.length; i++) {
			Car car = cars[i];
			if (car != null) {
				System.out.println(car);
			}
/*3번방법*/		//for (Car car  : cars) {
				//if (car != null) {
					//System.out.println(car);
				}
...
/*
 * 1) 가장 먼저 "현재 등록된 차량 0대"와 같은 메시지를 출력합니다. 2) 주차장이 가득찬 경우 '더 이상 차량 등록이
 * 불가능합니다.'메시지를 출력하고 addCar() 메소드를 종료한다. 3) sc 인스턴스를 이용해서 차량번호화 모델명을 입력 받은 뒤
 * cars 배열에 순차적으로 저장한다. 4) 차량번호와 모델명은 공백 없는 문자열로 처리한다. 5) 차량번호가 "288러1111"과 같은
 * 차량의 등록이 성공하면 "차량번호 288러1111 차량이 등록되었습니다." 메시지를 출력한다. 6) 기타 명시되지 않은 예외는 발생하지
 * 않는것으로 가장한다.
 */